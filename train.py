...
#                                                   Training
######################################################################################################################
def train(dataLoader, model_G, model_D, epoch,loss_G_real_plot,loss_D_real_plot,loss_D_fake_plot):
...
#         local_input3ch = input3ch_var[:,:,bbox_c[0][1]:bbox_c[0][3], bbox_c[0][0]:bbox_c[0][2]]
        ...
        if i % CONFIG.LOGS.PRINT_FREQ == 0:
            ...
            loss_G_real_plot.append(losses_G_real.avg)
            loss_D_real_plot.append(losses_D_real.avg)
            loss_D_fake_plot.append(losses_D_fake.avg)

             #loss_G_real_plot.append(losses_G_real.avg.data[0].tolist())
             #loss_D_real_plot.append(losses_D_real.avg.data[0].tolist())
             #loss_D_fake_plot.append(losses_D_fake.avg.data[0].tolist())
...
#                        normalize=True, range=None, scale_each=True, pad_value=0)
    ...
    #print(type(losses_G_real.avg.data[0].tolist()))

    epoch_count=range(1,len(loss_G_real_plot)+1)
    plt.plot(epoch_count, loss_G_real_plot, 'b-')
    plt.plot(epoch_count, loss_D_real_plot, 'r-')
    plt.plot(epoch_count, loss_D_fake_plot, 'r--')
    plt.legend(['G Loss','D_real_loss','D_fake_loss'])
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.savefig('loss_images/loss_epoch_'+str(epoch)+'.png')
    plt.close()
...
def main():
    ...
    for epoch in range(epoches):
        ...
        train(dataLoader, model_G, model_D, epoch,loss_G_real_plot,loss_D_real_plot,loss_D_fake_plot)
